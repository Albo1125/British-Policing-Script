...
internal class Main : Plugin
    {
        ...
        internal static string[] AudioFilesToCheckFor = new string[] { "LSPDFR/Police Scanner/British Policing Script Audio/Crimes/BRITISHCRIME_FAILTOSTOP.wav" };
        ...
        internal static Version TrafficPolicerVersion = new Version("6.13.6.0");
        internal static Version ArrestManagerVersion = new Version("7.6.8.0");
        internal static Version LSPDFRPlusVersion = new Version("1.6.1.0");
            ...
            if (onDuty)
            {
                ...
                if (Albo1125.Common.DependencyChecker.DependencyCheckMain(PluginName, Albo1125CommonVer, MinimumRPHVersion, MadeForGTAVersion, MadeForLSPDFRVersion, RAGENativeUIVersion, AudioFilesToCheckFor, OtherFilesToCheckFor))
                {
                    ...
                    if (!Albo1125.Common.DependencyChecker.CheckIfFileExists("Plugins/LSPDFR/Traffic Policer.dll", TrafficPolicerVersion))
                    {
                        ...
                        Albo1125.Common.CommonLibrary.ExtensionMethods.DisplayPopupTextBoxWithConfirmation("British Policing Script Dependencies", "BritishP.S. didn't detect Traffic Policer or detected Traffic Policer version lower than " + TrafficPolicerVersion.ToString() + ". Please install the appropriate version of Traffic Policer (link on the BPS download page under Requirements). Unloading British Policing Script...", true);
                        return;
                    }
                    if (!Albo1125.Common.DependencyChecker.CheckIfFileExists("Plugins/LSPDFR/Arrest Manager.dll", ArrestManagerVersion))
                    {
                        ...
                        Albo1125.Common.CommonLibrary.ExtensionMethods.DisplayPopupTextBoxWithConfirmation("British Policing Script Dependencies", "BritishP.S. didn't detect Arrest Manager or detected Arrest Manager version lower than " + ArrestManagerVersion.ToString() + ". Please install the appropriate version of Arrest Manager (link on the BPS download page under Requirements). Unloading British Policing Script...", true);
                        return;
                    }
                    if (!Albo1125.Common.DependencyChecker.CheckIfFileExists("Plugins/LSPDFR/LSPDFR+.dll", LSPDFRPlusVersion))
                    {
                        ...
                        Albo1125.Common.CommonLibrary.ExtensionMethods.DisplayPopupTextBoxWithConfirmation("British Policing Script Dependencies", "BritishP.S. didn't detect LSPDFR+ or detected LSPDFR+ version lower than " + LSPDFRPlusVersion.ToString() + ". Please install the appropriate version of LSPDFR+ (link on the BPS download page under Requirements). Unloading British Policing Script...", true);
                        return;
                    }
                    GameFiber.StartNew(delegate
                    {
                        int WaitCount = 0;
                        while (!IsLSPDFRPluginRunning("Traffic Policer", Main.TrafficPolicerVersion) || !IsLSPDFRPluginRunning("Arrest Manager", Main.ArrestManagerVersion) || !IsLSPDFRPluginRunning("LSPDFR+", Main.LSPDFRPlusVersion))
                        {
                            GameFiber.Yield();
                            WaitCount++;
                            if (WaitCount > 1500)
                            {
                                ...
                                Game.LogTrivial("TP: " + IsLSPDFRPluginRunning("Traffic Policer", Main.TrafficPolicerVersion).ToString());
                                Game.LogTrivial("AM: " + IsLSPDFRPluginRunning("Arrest Manager", Main.ArrestManagerVersion).ToString());
                                Game.LogTrivial("LSPDFRPLUS: " + IsLSPDFRPluginRunning("LSPDFR + ", Main.LSPDFRPlusVersion).ToString());
                                return;
                            }
                        }
